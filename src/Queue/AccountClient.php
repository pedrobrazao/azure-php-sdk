...
use AzurePhp\Storage\Queue\Model\QueueList;
...
final readonly class AccountClient
{
    ...
    public function getQueueClient(string $queueName): QueueClient
    {
        ...
        return new QueueClient($this->client, $uri);
    }
     ...
     */
    public function listQueues(string $prefix = '', bool $includeMetadata = false): \Generator
    {
        ...
        while (true) {
            ...
            $queueList = QueueList::fromXml($xml);

            foreach ($queueList->queues as $queue) {
                yield $queue;
            }

            $nextMarker = $queueList->nextMarker;
